...
appId=$(az ad sp create-for-rbac -n 'ACS Teams Interop App test 7' --query "appId" --only-show-errors --output tsv)
password=$(az ad sp create-for-rbac -n 'ACS Teams Interop App test 7' --query "password" --only-show-errors --output tsv)
