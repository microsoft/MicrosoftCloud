...
1. `samples/acs-video-to-teams-meeting/server/csharp/ExerciseACS.sln` を Visual Studio 2022 で開きます。

2. ソリューション エクスプローラーの `ExerciseACS` プロジェクトの上で右クリックします。
...
14. `+ 追加` ボタンを押して以下のキーと値を `アプリケーション設定` に追加してください。これらの値は Visual Studio の `ExerciseACS` プロジェクトの `local.settings.json` から取得してください。
